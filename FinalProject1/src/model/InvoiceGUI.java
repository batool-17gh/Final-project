...
public class InvoiceGUI extends JFrame {
    private InvoiceManager manager = new InvoiceManager(100);
...

    {
        ...
        tfMerchantOrMonth.addFocusListener(new java.awt.event.FocusAdapter() {
            ...
            public void focusGained(java.awt.event.FocusEvent e) {
                // إذا النص هو القيمة الافتراضية كاملة، نمسحها كلها
    ...
    private DefaultTableModel tableModel = new DefaultTableModel(
        ...
        public boolean isCellEditable(int row, int column) {
            return false; // يمنع تعديل الخلايا كلها
        }
    };
    ...
    private void addInvoice() {
        try {
            ...
            if (amount <= 0 || tax < 0) {
                JOptionPane.showMessageDialog(this, "Amount must be > 0 and tax >= 0.", "Input Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
    ...
    private void showInvoiceDetails(Invoice invoice) {
        ...
        // جدول واحد فقط فيه الفاتورة المطلوبة
        ...
        // الشريط السفلي فيه المجموع والزر
        JPanel bottomPanel = new JPanel(new BorderLayout());

        JLabel lblTotal = new JLabel(String.format(java.util.Locale.US, "Total: %.2f", invoice.getTotalAmount()));
        lblTotal.setBorder(BorderFactory.createEmptyBorder(5, 15, 5, 5));
        ...
        btnExport.addActionListener(e -> {
            ...
            try (PrintWriter pw = new PrintWriter(filename)) {
                ...
                pw.printf(java.util.Locale.US, "%d;\"%s\";\"%s\";%.2f;%.2f;%.2f%n",
                        ...
                        invoice.getTax(),
                        invoice.getTotalAmount());
                JOptionPane.showMessageDialog(dialog, "Invoice exported successfully.", "Export Complete", JOptionPane.INFORMATION_MESSAGE);
            } catch (IOException ex) {
                JOptionPane.showMessageDialog(dialog, "Error exporting invoice: " + ex.getMessage(), "Export Error", JOptionPane.ERROR_MESSAGE);
            }
        });
    ...
    private void calculateByMerchantAndOrMonth() {
        ...
        // ...
        private String lastLoadedFilePath = null;  // لازم يكون هنا
    ...
    private void loadFromFile() {
        ...
        // تحقق اذا تم تحميل نفس الملف مسبقا
    ...
    private void deleteSelectedInvoices() {
        ...
        // احذف من الأكبر للأصغر عشان حذف الصفوف ما يغير اندكسات الباقيين
        for (int i = selectedRows.length - 1; i >= 0; i--) {
            int modelRow = table.convertRowIndexToModel(selectedRows[i]);
            int invoiceNumber = (int) tableModel.getValueAt(modelRow, 0);

            manager.deleteInvoice(invoiceNumber);  // حذف من البيانات

            tableModel.removeRow(modelRow);  // حذف من الجدول
        }
